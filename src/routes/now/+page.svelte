...
<ul>
  ...
  {#if starTrek.previousSeriesComplete && starTrek.seriesComplete}
  <li><strong>series complete!</strong> {starTrek.previousSeriesComplete.series}: all {starTrek.previousSeriesComplete.totalEpisodes} episodes watched and journaled ({starTrek.previousSeriesComplete.journalEntries} journal entries). <strong>{starTrek.series}</strong>: all episodes complete ({starTrek.currentSeriesJournals} journal entries) — finished with "{starTrek.latestEpisodeTitle}" {starTrek.latestEpisodeSeasonEp} on {starTrek.lastUpdated ? starTrek.lastUpdated.slice(0, 10) : ""}.{#if starTrek.nextSeries} next up: <strong>{starTrek.nextSeries.series}</strong> ({starTrek.nextSeries.totalEpisodes} episodes).{/if} <a href="/series#star-trek">{starTrek.totalEpisodesWatched} episodes journaled</a>{#if daysSinceDeploy > 7} <span class="badge-stale">📡 snapshot from deploy &mdash; progress continues</span>{/if}</li>
