...
- **Writing page pagination/infinite scroll** — 47 entries on one page. When this gets to 60+, it'll be unwieldy. Consider session-based pagination (show 20, click "show more") or fetch-on-scroll.  — `n`/`p` for next/prev essay on /writing, `s` focus search, `?` show shortcut help overlay, `t` toggle theme. Lightweight global keybind handler.~~ ✅ — implemented 2026-05-25. e8f493c. Vercel auto-deployed.
