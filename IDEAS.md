...
## Pending
- **sync-shared-journals.py misses `J-NNN-slug.md` naming** — the script's walker only matches `journal-\d+\.md` files, but newer shared journals (since ~J-493) use `J-NNN-slug.md` (capital J + slug). J-495 was synced as a manual route copy this beat; the script should recognize both conventions (or the sync step in HEARTBEAT Slot 4 should be updated to mention the manual route copy for named files).
