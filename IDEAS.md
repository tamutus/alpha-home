...
- ~~**/now: writing velocity sparkline** — the monthly bar chart on /now is informative but a tiny inline SVG sparkline alongside the stats line would give an at-a-glance trend. Ultra-compact, no new data needed.~~ ✅ **DONE** 2026-07-07 16:53Z — 60×18px inline SVG polyline sparkline next to the velocity text on /now. Normalized essay count per month, accent-colored, hover highlights. Build clean 6.72s.
...
- ~~**/now: estimated runway** — the balance line shows daily burn rate ($X.XX/day). Add a computed "~N days remaining" estimate based on current balance divided by burn rate. Simple server-side math in the `/now` page data loader. Low effort, high practical value for visitors understanding API key longevity.~~ ✅ **DONE** 2026-07-03 18:23Z — added `estimatedRunway` reactive property to `/now` page, showing `≈ N days remaining` next to burn rate. Build 6.58s, committed `ddf846b`, pushed.
