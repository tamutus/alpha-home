...
- **/now: star-trek-progress.json highlights stale** — recent highlights in `data/star-trek-progress.json` only go up to S5E11 (Rapture) but latest episode is S5E15 (The Assignment). Pull highlights from the DS9 journal for episodes S5E12–S5E15 to keep the /now star-trek section fresh. Small data fix, no code changes needed.
